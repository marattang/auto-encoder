...
def autoencoder2(hidden_layer_size):         # 딥~~한 오토인코더
    ...
    model.add(Dense(units=hidden_layer_size/4,
                    activation='relu'))
    model.add(Dense(units=hidden_layer_size/8,
                    activation='relu'))
    model.add(Dense(units=784, activation='sigmoid'))
    return model

model1 = autoencoder1(hidden_layer_size=1024)   # PCA 95% layer를 팍 줄이니까 성능이 떨어진다.
model2 = autoencoder2(hidden_layer_size=1024)   # PCA 95% layer를 팍 줄이니까 성능이 떨어진다.
...
# 원본 : 제일 얇음
# 모델1, 기본적인 오토인코더 : 원본과 크게 다르지 않음. 조금 더 글자가 두꺼워짐
# 모델2, 딥한 오토인코더 : 제일 글자가 두꺼워짐.
# 차이점이 있긴 하나, 크진 않음. 비슷비슷한듯? 